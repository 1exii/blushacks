...
function createRouter(init) {
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    if (opts && opts.pendingError) {
      ...
      let actionResult = await handleAction(request, location, opts.submission, matches, scopedContext, fogOfWar.active, { replace: opts.replace, flushSync: flushSync2 });
...
async function callDataStrategyImpl(dataStrategyImpl, type, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties2, scopedContext, enableMiddleware) {
  ...
  let dsMatches = matches.map((match, i) => {
    ...
    let resolve = async (handlerOverride) => {
      ...
      return shouldLoad ? callLoaderOrAction(type, request, match, loadRoutePromise, handlerOverride, scopedContext) : Promise.resolve({ type: "data", result: void 0 });
    };
    return {
      ...match,
      shouldLoad,
...
function Links() {
  ...
  return React9.createElement(React9.Fragment, null, typeof criticalCss === "string" ? React9.createElement("style", { dangerouslySetInnerHTML: { __html: criticalCss } }) : null, typeof criticalCss === "object" ? React9.createElement("link", { rel: "stylesheet", href: criticalCss.href }) : null, keyedLinks.map(({ key, link }) => isPageLinkDescriptor(link) ? React9.createElement(PrefetchPageLinks, { key, ...link }) : React9.createElement("link", { key, ...link })));
}
...
function createRoutesStub(routes, unstable_getContext) {
  return function RoutesTestStub({
    ...
    if (routerRef.current == null) {
      ...
      let patched = processRoutes(convertRoutesToDataRoutes(routes, (r) => r), remixContextRef.current.manifest, remixContextRef.current.routeModules);
...
function createHydratedRouter({
  ...
  if (ssrInfo.context.isSpaMode) {
    ...
    let initialMatches = matchRoutes(routes, window.location, (_b = window.__reactRouterContext) == null ? void 0 : _b.basename);
    if (initialMatches) {
